...
def mostrar_reglas(operadores, proposiciones_simples):
    ...
    def corregir_proposicion():
        ...
        def aplicar_correcion():
            nueva_proposicion = texto_reglas.get("2.0", tk.END).strip()
            if nueva_proposicion:
                entrada_proposicion.delete(0, tk.END)  # Limpiar la entrada original
                entrada_proposicion.insert(0, nueva_proposicion)  # Insertar la nueva proposición
                ventana_reglas.destroy()  # Cerrar ventana de reglas

        # Botón para aplicar la corrección
        boton_aplicar = tk.Button(ventana_reglas, text="Aplicar Corrección", command=aplicar_correcion)
        boton_aplicar.pack()

    # Botón para corregir la proposición
